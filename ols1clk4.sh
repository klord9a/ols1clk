...
WPNUM=1
WPTHEME='NULL'
WORDPRESSPATH=$SERVER_ROOT/wordpress_$WPNUM
SITEDOMAIN=*
EMAIL=
ADMINPASSWORD=
ROOTPASSWORD=
...
function install_wordpress_core
{
    ...
    wp plugin install 'https://drive.google.com/uc?id=13fPuXHRBWbb66Vce6x3Za5xYZ2lS01mk&export=download&confirm=1' --allow-root --activate --quiet
    wp plugin install 'https://drive.google.com/uc?id=1GKDRG8haB21p3wNAnSNnvynPLMEnp_kE&export=download&confirm=1' --allow-root --activate --quiet
    wp plugin install 'https://drive.google.com/uc?id=1tZGJXGkG7y6tXTIo_kVVgRAa2Tl3t5EO&export=download&confirm=1' --allow-root --activate --quiet
    # theme
...
function config_vh_wp
{
    ...
    if [ -e "${WEBCF}" ] ; then
        cat ${WEBCF} | grep "virtualhost wordpress_$WPNUM" >/dev/null
        if [ $? != 0 ] ; then
            sed -i -e "s/adminEmails/adminEmails $EMAIL\n#adminEmails/" "${WEBCF}"
            sed -i -e "s/ls_enabled/ls_enabled   1\n#/" "${WEBCF}"
...
END
            echoB "${FPACE} - Create wordpress_$WPNUM virtual host conf"
            mkdir -p $SERVER_ROOT/conf/vhosts/wordpress_$WPNUM/
            cat > $VHOSTCONF <<END
docRoot                   \$VH_ROOT/
index  {
  useServer               0
...
END
            ...
            echoY "${FPACE} - Detect wordpress_$WPNUM exist, will skip virtual host conf setup!"
        fi
    else
        echoR "${WEBCF} is missing. It appears that something went wrong during OpenLiteSpeed installation."
        ALLERRORS=1
...
function test_wordpress
{
    ...
    test_page http://localhost:$WPPORT/ "WordPress" "test wordpress_$WPNUM HTTP first page"
    test_page https://localhost:$SSLWPPORT/ "WordPress" "test wordpress_$WPNUM HTTPS first page"
}

function test_wordpress_plus
{
    test_page http://localhost:8088/  Congratulation "test Example vhost page"
    test_page http://$SITEDOMAIN:$WPPORT/ WordPress "test wordpress_$WPNUM HTTP first page"
    test_page https://$SITEDOMAIN:$SSLWPPORT/ WordPress "test wordpress_$WPNUM HTTPS first page"
}
...
while [ ! -z "${1}" ] ; do
    case "${1}" in
        ...
        --sitenum )       
                ...
                ;;
        --sitetheme )       
                check_value_follow "$2" "WordPress website theme"
                shift
                WPTHEME=$FOLLOWPARAM
                ;;
        -[Uu] | --uninstall )       
