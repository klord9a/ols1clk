...
WPNUM='wordpress'
WPTHEME='NULL'
WORDPRESSPATH=$SERVER_ROOT/$WPNUM
SITEDOMAIN=*
EMAIL=
ADMINPASSWORD=
ROOTPASSWORD=
...
function install_wordpress_core
{
    ...
    echoG "Install $WPNUM Sll plugin"    
    ...
    # normal plugins:
    echoG "Install $WPNUM normal plugins"  
    wp plugin install \
    all-in-one-wp-migration \
        --allow-root \
        --quiet
    echoG "Install $WPNUM activate plugins"  
    # loco-translate \
    # gtranslate \
    ...
    easy-theme-and-plugin-upgrades \
        ...
        --quiet
    # prime plugins:
    echoG "Install $WPNUM prime plugins"  
    ...
    # theme
    echoG "Install $WPNUM THEME"
    wp theme install $WPTHEME --activate --allow-root
    echoG "Install $WPNUM Cache plugin"    
...
function config_vh_wp
{
    ...
    if [ -e "${WEBCF}" ] ; then
        cat ${WEBCF} | grep "virtualhost $WPNUM" >/dev/null
        if [ $? != 0 ] ; then
            sed -i -e "s/adminEmails/adminEmails $EMAIL\n#adminEmails/" "${WEBCF}"
            sed -i -e "s/ls_enabled/ls_enabled   1\n#/" "${WEBCF}"

            VHOSTCONF=$SERVER_ROOT/conf/vhosts/$WPNUM/vhconf.conf
            ...
            # else   
                echoB "${FPACE} - Create $WPNUM listener"  
                cat >> ${WEBCF} <<END

listener $WPNUM {
address                 *:$WPPORT
secure                  0
map                     $WPNUM $SITEDOMAIN
}
...
END
            # fi
            echoB "${FPACE} - Insert $WPNUM virtual host"  
            cat >> ${WEBCF} <<END
...
END
            echoB "${FPACE} - Create $WPNUM virtual host conf"
            mkdir -p $SERVER_ROOT/conf/vhosts/$WPNUM/
            cat > $VHOSTCONF <<END
docRoot                   \$VH_ROOT/
index  {
  useServer               0
...
END
            ...
            echoY "${FPACE} - Detect $WPNUM exist, will skip virtual host conf setup!"
        fi
    else
        echoR "${WEBCF} is missing. It appears that something went wrong during OpenLiteSpeed installation."
        ALLERRORS=1
...
function test_wordpress
{
    ...
    test_page http://localhost:$WPPORT/ "WordPress" "test $WPNUM HTTP first page"
    test_page https://localhost:$SSLWPPORT/ "WordPress" "test $WPNUM HTTPS first page"
}

function test_wordpress_plus
{
    test_page http://localhost:8088/  Congratulation "test Example vhost page"
    test_page http://$SITEDOMAIN:$WPPORT/ WordPress "test $WPNUM HTTP first page"
    test_page https://$SITEDOMAIN:$SSLWPPORT/ WordPress "test $WPNUM HTTPS first page"
}
